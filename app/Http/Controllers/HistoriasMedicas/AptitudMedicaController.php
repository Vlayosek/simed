...
class AptitudMedicaController extends Controller
{
    ...
    public function datatableAptitudesMedicas($identificacion, $id)
    {
        $data = AptitudMedica::select(
            'id',
            'codigo',
            'aptitud',
            'observacion',
        ...
        return (new CollectionDataTable($data))
            ...
            ->addColumn('', function ($row) {
                ...
                $btn .= '<tr><td style="padding: 2px;border:0px"><a title="Editar" class="btn btn-block btn-outline-primary  btn-xs"  href="#modal-aptitud_medica" role="button" data-toggle="modal"  onclick="app_aptitudes_medicas.editarRegistro(\'' . $row->id . '\')"><i class="fa fa-cog"></i>&nbsp;Editar</a></td></tr>';
                $btn .= '<tr><td style="padding: 2px;border:0px"><button title="Eliminar" class="btn btn-block btn-outline-danger  btn-xs"  onclick="app_aptitudes_medicas.eliminar(\'' . $row->id . '\')"><i class="fa fa-trash"></i>&nbsp;Eliminar</button></td></tr>';
                $btn .= '</table>';
                return $btn;
            })
            ->rawColumns([''])
    ...
    {
        $data = AptitudMedica::whereIn('codigo', AtencionMedica::select('codigo')->where('identificacion', $request->identificacion)->where('codigo', $request->codigo)->pluck('codigo'))
            ->where('eliminado', false)
            ->get();
        ...
        try {
            ...
            if ($request->id != 0) {
                ...
                $cql = AptitudMedica::find($request->id);
    ...
    {
        $array_response['datos'] = AptitudMedica::where('id', $request->id)->first();
        $array_response['status'] = 200;
        return response()->json($array_response, 200);
    }
